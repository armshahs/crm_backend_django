...
def get_stripe_pub_key(request):
    ...
    return Response({"pub key": pub_key})
...
# post request for plans upgrade
@api_view(["POST"])
...
# the below code works using email as a filter
# @api_view(["POST"])
...
# the below code works using username as a filter
@api_view(["POST"])
def add_member(request):
    team = Team.objects.filter(members__in=[request.user]).first()
...
def stripe_webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        team = Team.object.get(pk=session.get('client_reference_id'))
